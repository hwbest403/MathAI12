...
#test1=load_kor_data(path+"test_easy_1.json")
#test1=load_kor_data(path+"test_easy_3.json")
#test1=load_kor_data(path+"test_easy_4.json")
#test1=load_kor_data(path+"test_easy_6.json")

(generate_nums, copy_nums) = read_pickle('generate_nums_easy.p')
input_lang = read_pickle('input_lang_easy.p')
output_lang = read_pickle('output_lang_easy.p')
...
# 모델 초기화
encoder.load_state_dict(torch.load(model_path + "encoder_easy"))
predict.load_state_dict(torch.load(model_path + "predict_easy"))
generate.load_state_dict(torch.load(model_path + "generate_easy"))
merge.load_state_dict(torch.load(model_path + "merge_easy"))
generate_num_ids = []
...
for test_batch in test_pairs:
    ...
    try:
        ...
        if abs(exec_vars['final_result']- test_batch[9])<1e-4:
            value_ac += 1
    except:
        pass
